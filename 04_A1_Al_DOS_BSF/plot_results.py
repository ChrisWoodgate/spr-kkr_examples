...
# Parse the BSF data
bsfdata = sprkkr_bsf_plotting.parse_bsfdata('blochsf/_Al_BSFEK_BLOCHSF.bsf')

l_max = 2

# Parse the DOS data
re_energies, dos, species_resolved, lm_resolved, title, elements, concentrations, n_species = sprkkr_dos_plotting.sprkkr_dos_read('dos/_Al_DOS_DOS.dos', l_max=l_max, spin_channels=2)
...
for i in range(0, n_species,2):
    ax2.plot(species_resolved[i,:], re_energies, alpha=1.0, label=elements[i], color='tab:grey')
ax2.legend(fontsize=11, loc='lower right')

plt.tight_layout()
plt.savefig('./Al.pdf', dpi=300)
plt.close()
...
ax.set_title(r'Al DOS: $l$-resolved')

plt.tight_layout()
plt.savefig('./Al_l-resolved.pdf')
plt.close()
